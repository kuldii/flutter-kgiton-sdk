# Authorization & Licensing

## 🔐 This is Proprietary Software

**KGiTON SDK** is commercial software owned by PT KGiTON. Use of this SDK requires explicit authorization.

---

## How to Obtain Authorization

### Step 1: Contact PT KGiTON

Send an email to **support@kgiton.com** with the following information:

**Subject**: KGiTON SDK License Request

**Required Information**:
- Company/Organization Name
- Contact Person Name and Title
- Business Email Address
- Phone Number
- Intended Use Case (brief description)
- Estimated Project Timeline
- Number of Developers

### Step 2: Review License Agreement

PT KGiTON will send you:
1. Commercial License Agreement
2. Pricing information
3. Terms and conditions
4. Service Level Agreement (SLA)

### Step 3: Sign Agreement

After reviewing:
1. Sign the license agreement
2. Complete payment (if applicable)
3. Provide necessary business documentation

### Step 4: Receive Access

Once authorized, you will receive:
- ✅ License ID and credentials
- ✅ Access to private repository (if applicable)
- ✅ SDK documentation
- ✅ Technical support contact
- ✅ Sample projects and examples

---

## License Types

### Development License
- For development and testing purposes
- Limited to development environment
- Includes technical support
- Valid for specified duration

### Commercial License
- For production deployment
- Unlimited devices (or as specified)
- Priority technical support
- Includes updates and patches
- Annual or perpetual options

### Enterprise License
- Custom terms and pricing
- Dedicated support channel
- Custom feature development options
- On-premise deployment options
- Training and consultation included

---

## What's Included

### With Your License

✅ Full SDK source code access  
✅ Documentation and API reference  
✅ Example applications  
✅ Technical support via email  
✅ Bug fixes and security patches  
✅ Minor version updates  

### Additional Services (Optional)

- 🔧 Custom integration assistance
- 📚 On-site training
- 🛠️ Custom feature development
- 🔒 Extended support plans
- 📱 White-label options

---

## Terms & Conditions Summary

### You MAY:
- ✅ Use the SDK for authorized projects
- ✅ Integrate into your commercial applications
- ✅ Receive updates during license period
- ✅ Contact support for technical assistance

### You MAY NOT:
- ❌ Share or redistribute the SDK
- ❌ Reverse engineer or decompile
- ❌ Remove proprietary notices
- ❌ Sublicense to third parties
- ❌ Use beyond authorized scope
- ❌ Disclose confidential information

---

## Verification

To verify a license:

1. Visit: https://kgiton.com/verify-license
2. Enter your License ID
3. Check license status and validity

---

## Support Channels

### For Licensed Users

**Technical Support**:  
Email: support@kgiton.com  
Response Time: Within 24 hours

**Sales & Licensing**:  
Email: sales@kgiton.com  
Phone: [Your Phone Number]

**Security Issues**:  
Email: security@kgiton.com  
(For authorized users only)

### Business Hours

Monday - Friday: 9:00 AM - 5:00 PM WIB (GMT+7)  
Emergency Support: Available for Enterprise license holders

---

## FAQ

**Q: Is there a free trial?**  
A: Contact us to discuss evaluation options for qualified organizations.

**Q: What happens when my license expires?**  
A: You can renew your license to continue receiving updates and support. Existing deployments continue to function.

**Q: Can I use this SDK in open-source projects?**  
A: No, this SDK cannot be used in open-source projects. Contact us for special arrangements.

**Q: Do you offer discounts for startups or educational institutions?**  
A: Yes, we offer special pricing. Contact sales@kgiton.com with details about your organization.

**Q: Can I transfer my license?**  
A: License transfers require approval from PT KGiTON. Contact support@kgiton.com.

---

## Legal

Copyright (c) 2025 PT KGiTON. All Rights Reserved.

This software is protected by copyright laws and international treaties. Unauthorized reproduction or distribution may result in severe civil and criminal penalties.

For complete legal terms, see [LICENSE](LICENSE) file.

---

**Ready to get started?**  
📧 Contact us: support@kgiton.com  
🌐 Visit: https://kgiton.com
